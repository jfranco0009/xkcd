...
#' given an [`xkcd`] comic number this function retrieves and JSON object
#' describing that comic from the xkcd website
#' @param number A scalar numeric vector identifying  the xkcd comic
#'
...
#' * day
#'
#'Note that many of these field may be an empty string
#'
#' @export
...
validate_xkcd <- function(x){
  ...
  if(!all(required_fields %in% names(x))) {
    ...
    stop("xkcd is missing the following required fields: ")
...
#' xkcd viz
#'
#' visualise xkcd comics
#'
#' given an [`xkcd`] object, this [`base::plot`] method retrieves the image file
#' associated with this comic from the xkcd website and displays it in the currently
...
if(img_type == "png"){
  p <- png::readPNG(tmp)
} else if(img_type == "jpeg" || img_type == "jpg") {
  p <- jpeg::readJPEG(tmp)
} else {
  stop("Unknow image format", img_type)
}
  plot.new()
  grid::grid.raster(p)
}
